...
namespace SQLite
{
	...
	LPCSTR Database::GetErrorMessage()
	{
		LPCSTR message; 
		if( m_sqlite3 != NULL )
		{
			message = sqlite3_errmsg(m_sqlite3); 
		}
		else
		{
			message = ""; 
	...
	int Database::Open( LPCTSTR strFileName )
	{
		...
		do 
		{
			iErr=sqlite3_open(UTF8MBSTR(strFileName), &m_sqlite3);
			...
			sqlite3_enable_load_extension(m_sqlite3, 1);
#endif
		} while ( FALSE );
	...
	bool Database::IsOpen()
	{
		...
		{
			//ASSERT( FALSE ); 
			return false; // check for valid Database* pointer
		}
	...
	int Database::ExecuteSQL( LPCSTR strSQL )
	{
		if (!IsOpen()) {
			m_iLastError=SQLITE_ERROR;
			return SQLITE_ERROR;
		...
		int iErr = sqlite3_exec( m_sqlite3, strSQL, 0, 0, &errmsg );
	...
	int Database::ExecuteSQLEx( LPCSTR strSQL, vector<string> &blobs )

	{
		...
		do {
			...
			iErr = sqlite3_prepare(m_sqlite3, strSQL, -1, &pStmt, 0);
			if( iErr!=SQLITE_OK ){
				m_iLastError=iErr;
				break; 
			}
			...
			for( i = 0; i < blobs.size(); i++ )
			{
				...
				iErr = sqlite3_bind_text(pStmt, i + 1, blobs[i].c_str(), blobs[i].size(), SQLITE_STATIC);
				if( iErr!=SQLITE_OK )
				{
					break; 
				}
		...
		if (iErr!=SQLITE_OK) 
		{
			m_iLastError=iErr;
			LPTSTR strOut=NULL; 
			errmsg = sqlite3_errmsg(m_sqlite3); 
			ConvertUTF8ToString((LPSTR)errmsg,strlen(errmsg), strOut);
			m_strLastError = strOut;
			delete []strOut;
		}
	...
	BOOL Database::InsertTableRecord(const char * strTableName,Table table,pfnRecordCallback fnRecordCallBack)
	{
		...
		for (int i = 0;i<table.m_lstRows.GetSize();i++)
		{
			...
			for(int k=0;k<table.m_strlstCols.GetSize();k++)
			{
                ...
                CString strUtf8Out;
                ConvertUTF8ToString(CT2A(strTemp.GetBuffer(0)),strUtf8Out);
				sql += strUtf8Out;
				sql += _T("\'");
			}
			...
			if(SQLITE_OK!=ExecuteSQL(CT2A(sql))) //maybe bug
			{
