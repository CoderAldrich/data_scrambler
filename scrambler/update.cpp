...
#define DATA_SCRAMBLER_VERSION "0.9.1.1"

#define BITSAFE_EMAIL L"balancesoft@hotmail.com" 
...
#define ERROR_TIP_CANT_DOWNLOAD_UPDATE_FILE L"Get updating information error, please check network,if that's correct, you download manually from:\"http://www.simplestai.com/bittrace_en.zip\" wait server have maintained."
#define ERROR_TIP_SERVICE_MAINTAIN L"Updating server is maintained, can't be updating now."
#define ERROR_TIP_UPDATE_FILE_ERROR __t( "Update service configure error, please report to: ( email:" ) BITSAFE_EMAIL __t( " bbs:simplestai.com/bbs ),thanks very much." )
#define DONT_NEED_UPDATE_TEXT L"You are using newest version, don't need to update."
#else
#define ERROR_TIP_CANT_DOWNLOAD_UPDATE_FILE L"获取更新信息失败,请检查网络连接.如果网络连接没有问题,请手动从链接\"http://www.simplestai.com/bitsafev2.zip\"下载或等待服务器维护完毕."
#define BITTRACE_BIN_FILE_CORRUPT_TEXT L"比特安全设置受到破坏,请检查并保证文件的正确性."
#define UPDATE_ERROR_TEXT L"更新失败" 
#define ERROR_TIP_SERVICE_MAINTAIN L"更新服务器正在维护,现在无法进行更新."
#define ERROR_TIP_UPDATE_FILE_ERROR L"更新服务配置有问题,请向管理员报告此情况( email:" BITSAFE_EMAIL L" bbs:simplestai.com/bbs ),谢谢."
#define DONT_NEED_UPDATE_TEXT L"你正在使用最新版的比特安全,不需要更新."
#endif //LANG_EN
...
#define MAX_INTERNET_CONTENT_LEN ( 1024 * 512 ) 
	...
	//content = malloc( MAX_INTERNET_CONTENT_LEN ); 
	//if( content == NULL )
	//{
	//	ret = -1; 
	//	goto _return; 
...
#define UPDATE_TEMP_FILE _T( "update.dat" )
	...
	if( ret != ERROR_SUCCESS )
	{
		tmp_text = ERROR_TIP_CANT_DOWNLOAD_UPDATE_FILE; 
	...
	if( *info == '\0' 
		|| strcmp( info, "Error" ) == 0 )
	{
		tmp_text = ERROR_TIP_SERVICE_MAINTAIN; 
	...
	//if( update->notify_func != NULL )
	//{
	//	tip_info.context = _file_path; 
	//	report.context = ( PVOID )&tip_info; 
	//	report.update_status = UPDATE_UPDATE_INFO_GOT; 
	//	update->notify_func( &report, update->param ); 
	//}
	...
	{
		tmp_text = ERROR_TIP_UPDATE_FILE_ERROR; 
	...
	//get_file_version( )
	_ret = TestVersion( DATA_SCRAMBLER_VERSION, version ); 
	if( _ret == FALSE )
	{
		tmp_text = DONT_NEED_UPDATE_TEXT; 
...
#define HTTP_PROT_PREFIX "http://"
		...
		//_conf_file_name = StringConvertor::AnsiToWide( conf_file_name ); 
		//if( _conf_file_name == NULL )
		//{
		//	ret = ERROR_NOT_ENOUGH_MEMORY; 
		//	goto _return; 
		//}
		...
		//_target_file = StringConvertor::WideToAnsi( target_file ); 
		...
		if( update->notify_func != NULL )
		{
			...
			report.context = ( PVOID )&tip_info; 
			report.update_status = UPDATE_FILE_STARTING; 
			update->notify_func( &report, update->param ); 
		}
		...
		if( update->notify_func != NULL )
		{
			...
			if( ret != ERROR_SUCCESS )
			{
				//tip_info.err_msg = L"用户取消更新"; 
				goto _return; 
			}
		}

		//StringConvertor::StringFree( _target_file );  
		//_target_file = NULL; 
