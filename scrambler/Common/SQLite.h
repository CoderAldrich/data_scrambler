...
//#include <string>
//#include <vector>
typedef struct tagMyString
{
	char* szStart;
	int nLen;
}MyString,*PMyString;
int split_string( char* szBuffer,int nBufferLen,const char* szSplitCharSet,PMyString pMyString,int nSize);
typedef ULONG (__stdcall* pfnSQLite3QueryCallback)(PVOID pvParam,const char** szReturnString,ULONG nRow,ULONG nCol);
namespace SQLite
{
	//////////////////////////////////////////////////////////////////////////
	// Typedefs
	...
	typedef CSimpleArray <CString> stdvstring; 
	typedef stdvstring vstrlist;
    ...
    typedef BOOL (WINAPI* pfnRecordCallback)(const char * strTableName,Table& table);
	// Main wrapper
	class Database
	{
	public:
		...
		void ClearError() { m_iLastError=SQLITE_OK;m_strLastError=_T(""); };
		...
		ULONG QuerySQL3(LPCTSTR strSQL,pfnSQLite3QueryCallback fn);
		...
		// Writes a BLOB bound using an SQL statement, 
		...
		//
		// Once a trbound igger terminates, the value returned by this routine 
		...
		void ConvertUTF8ToString( char * strInUTF8MB, size_t len, LPTSTR &  strOut );
		void ConvertUTF8ToString( char * strInUTF8MB, CString & strOut );

		BOOL ExportTableAsCSV(LPSTR strTableName,LPSTR strFileName,LPSTR strCond=NULL);
		BOOL ExportTableAsXML(LPSTR strTableName,LPSTR strFileName,LPSTR strCond=NULL);

		BOOL ImportTableAsCSV(LPSTR strTableName,LPSTR strFileName,pfnRecordCallback fnRecordCallBack=NULL);
		BOOL ImportTableAsXML(LPSTR strTableName,LPSTR strFileName,pfnRecordCallback fnRecordCallBack=NULL);

        BOOL ImportTableAsBinary(LPSTR strTableName,LPSTR strFileName,pfnRecordCallback fnRecordCallBack=NULL);
        BOOL ExportTableAsBinary(LPSTR strTableName,LPSTR strFileName,LPSTR strCond=NULL);
        BOOL ExportTableAsMemory(LPSTR strTableName,ULONG * pulRowCount,ULONG* pulColCount,CHAR*pszBuffer,ULONG* pulBufferLen,LPSTR strCond=NULL);


		BOOL IsTableExistd(const TCHAR* tszTableName );
        BOOL IsTableColumnExistd(const TCHAR* tszTableName,  const TCHAR* tszFieldName );

	private:
		sqlite3 * m_sqlite3;
		int m_iLastError;
		BOOL RealExportTableAsXML(LPSTR strTableName,LPSTR strFileName,LPSTR strCond=NULL);
        BOOL RealImportTableAsXML(LPSTR strTableName,LPSTR strFileName,pfnRecordCallback fnRecordCallBack=NULL);

		BOOL InsertTableRecord(const char * strTableName,Table table,pfnRecordCallback fnRecordCallBack = NULL);
	};
	...
	public:
		...
		bool GoRow(unsigned int iRow);
         ...
         int GetColIndex(LPCTSTR lpColName);
        ...
        LPCTSTR SetValue(int iColIndex,LPCTSTR lpstrValueName);
