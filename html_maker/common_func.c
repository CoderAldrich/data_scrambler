...
INT32 CheckFileExist( LPCWSTR FileName, ULONG Flags )
{
	INT32 Ret = 0; 
	ULONG dwRet; 
	...
	do 
	{
		...
		if( Flags & CHECK_DIRECTORY )
		{
			FileAttr = FILE_ATTRIBUTE_DIRECTORY | FILE_FLAG_BACKUP_SEMANTICS; 
		}
		else
		{
		...
		if( INVALID_HANDLE_VALUE == FileHandle ) 
		{
			...
			dwRet = GetLastError(); 
			log_trace_ex( MSG_ERROR, " check file %ws error %u\n", FileName, dwRet );
			break; 
		}
...
INT32 WINAPI create_directory_ex( LPWSTR file_path, ULONG cc_file_path_len, ULONG dir_begin_level )
{
	INT32 Ret = 0; 
	INT32 _Ret; 
	ULONG dwRet; 
	INT32 i; 
	ULONG PathDelimCount; 
	...
	do 
	{
		...
		_Ret = CheckFileExist( file_path, CHECK_DIRECTORY ); 
		if( _Ret == 0 )
		{
			Ret = _Ret; 
			break; 
		...
		log_trace_ex( MSG_WARNING, "install template directory %ws is not exist, create it\n", file_path ); 

		PathDelimCount = 0; 
		for( i = 0; ( ULONG )i < cc_file_path_len; i ++ )
		{
			if( file_path[ i ] == PATH_DELIM_CH )
			{
				PathDelimCount ++; 

				if( PathDelimCount > dir_begin_level )
				{
					file_path[ i ] = L'\0'; 

					do 
					{
						_Ret = CreateDirectoryW( file_path, NULL ); 
						if( _Ret == FALSE )
						{
							dwRet = GetLastError(); 
							...
							log_trace_ex( MSG_FATAL_ERROR, "create the install template directory %ws error %u\n", 
								file_path, 
								dwRet ); 
						}
					}while( FALSE );

					file_path[ i ] = PATH_DELIM_CH; 
...
INT32 error_handle( LPCSTR msg, DWORD mode, DWORD mb_flags )
{
	...
	//dbg_print( MSG_WARNING | DBG_DBG_BUF_OUT, "before FormatMessage last error is %d \n", GetLastError() ); 
	...
	//dbg_print( MSG_INFO | DBG_DBG_BUF_OUT, "after FormatMessage return %d last error is %d \n", msg_len, GetLastError() ); 
...
LRESULT dbg_print(
	...
	... )
{
#define MAX_DBG_OUT_LN_LEN 10240
...
#define MAX_FMT_STR_LEN 512
		...
		{
			INT32 _ret; 
			_ret = _vsnprintf( output, MAX_DBG_OUT_LN_LEN, dbg_msg, ap);
			if( _ret < 0 )
			{
				ASSERT( _ret == -1 ); 
				output[ MAX_DBG_OUT_LN_LEN - 1 ] = '\0'; 
			}
			else if( _ret >= MAX_DBG_OUT_LN_LEN )
			{
				output[ MAX_DBG_OUT_LN_LEN - 1 ] = '\0'; 
			}
#ifdef _DEBUG
...
#define MAX_DBG_OUT_LN_LEN 10240
	...
	ULONG level; 
	ULONG mode; 
	WCHAR output[ MAX_DBG_OUT_LN_LEN ]; 
		...
		|| ( ( level > DBG_LVL_VALUE_PART_MASK ) && ( ( level & ( ~DBG_LVL_VALUE_PART_MASK ) ) & dbg_print_lvl ) ) )
	{
		{
			HRESULT hr; 
			hr = StringCchVPrintf( output, MAX_DBG_OUT_LN_LEN, dbg_msg, ap);
			if( FAILED( hr ) )
			{
				//ASSERT( hr ==  ); 
				output[ MAX_DBG_OUT_LN_LEN - 1 ] = '\0'; 
...
LRESULT _mcbs_to_unicode( UINT32 code_page, LPCSTR mcbs, LPWSTR unicode, ULONG cch_len, PULONG ret_len )
{
	...
	need_len = MultiByteToWideChar( code_page, 0, mcbs, -1, NULL, 0 ); 
	...
	need_len = MultiByteToWideChar( code_page, 0, mcbs, -1, unicode, cch_len ); 
...
LRESULT _unicode_to_mcbs( UINT32 code_page, LPCWSTR unicode, LPSTR mcbs, ULONG cch_len, PULONG cc_ret_len )
{
	LRESULT ret = ERROR_SUCCESS; 
	DWORD cc_need_len;
	...
	cc_need_len = WideCharToMultiByte( code_page, 0, unicode, -1, NULL, 0, NULL, FALSE ); 
	
	if( cc_ret_len != NULL )
	{
		*cc_ret_len = cc_need_len; 
	}

	if( cc_need_len > cch_len 
		|| mcbs == NULL )
	{
		ret = ERROR_BUFFER_TOO_SMALL; 
		goto _return; 
	...
	cc_need_len = WideCharToMultiByte( code_page, 0, unicode, -1, mcbs, cch_len, NULL, FALSE ); 
	...
	if( cc_need_len == cch_len )
	{
		if( mcbs[ cc_need_len - 1 ] != '\0' )
		{
			DBG_BP(); 
			mcbs[ cc_need_len - 1 ] = '\0'; 
			ret = ERROR_BUFFER_OVERFLOW; 
		}
	}
#endif //_DEBUG
...
/** @brief 生成动态缓存区
* @param buf 动态缓存区指针，输出
* @param len 动态缓存区长度，输出
...
/** @brief 计算缓存区的长度是否合乎要求，如不小于要求长度，重新分配
* @param buf 当前的缓存区指针/输出的新生成的缓存区指针
* @param len 当前缓存区已经使用长度
* @param max_len 当前缓存区的分配长度
* @param need_len 需要使用的长度
* @return 如果成功 返回STATUS_SUCCESS，否则返回错误 码。
*/
...
BOOL SelfDeleteASM()
{
	...
	//以下这段代码可能和操作系统相关(98/2000/xp), 有些代码要做调整
#ifndef _WIN64
	__asm 
	{
		lea eax, buf
...
#define CMD_LINE_LEN 320
	...
	if( proc_id != NULL )
	{
		*proc_id = clone_process_info.dwProcessId; 
	}
...
LRESULT wait_thread_end( HANDLE thread_handle, ULONG wait_time )
{
		...
		WAIT_ABANDONED != wait_ret )
	{
		...
		_ret = TerminateThread( thread_handle, 0xeeee ); 

		if( FALSE == _ret )
		{
			dbg_print( MSG_FATAL_ERROR | DBG_MSG_AND_ERROR_OUT, "terminate thread error\n" ); 
		}
	}
									  ...
									  ULONG data_size )
{
	...
	do 
	{
		...
		buff = ( BYTE* )malloc( sizeof( BYTE ) * len ); 
		if( NULL == buff )
		{
			break; 
		}
		...
		CopyMemory( buff, data, data_size ); 

		cpd.dwData = 0;
		cpd.cbData = len;
		cpd.lpData = ( PVOID )buff;

		_ret = SendMessage( taget_wnd, WM_COPYDATA, ( WPARAM )taget_wnd, (LPARAM)&cpd); 
		if( FALSE == _ret )
		{
			SAFE_SET_ERROR_CODE( ret ); 
			dbg_print( MSG_IMPORTANT | DBG_MSG_AND_ERROR_OUT, "send copy data message error\n"); 
		}
...
LRESULT WINAPI open_folder_dlg( LPWSTR folder_path, ULONG cc_buf_len )
{
	...
	do 
	{
		...
		bi.lpszTitle      = NULL; //TEXT( "请选择文件夹" ); 
		bi.ulFlags        = BIF_RETURNONLYFSDIRS | BIF_STATUSTEXT | BIF_USENEWUI; 
		bi.lpfn           = NULL; //folder_browser_callback; //回调函数
		bi.lParam         = NULL; //(LPARAM)TEXT("D:\\"); //传给回调函数的参数,设置默认路径
		bi.iImage         = 0; 
					   ...
					   VS_FIXEDFILEINFO* FixedFileInfo )
{
	...
	do 
	{
		...
		if( FileVersionSize == 0 )
		{
			...
			//DbgPrintEx( "get file %ws version information size error %u\n", 
			//	FileName, 
			//	ErrorCode ); 
		...
		_Ret = GetFileVersionInfo( FileName, 0, FileVersionSize, VersionInfo );

		if( _Ret == FALSE )
		{
			ErrorCode = GetLastError(); 

			//DbgPrintEx( "get file %ws version information error %u\n", 
			//	FileName, 
			//	ErrorCode ); 
...

		{
			...
			if( _Ret == FALSE )
			{
				...
				//DbgPrintEx( "get the detail of the file %ws version information error %u\n", 
				//	FileName, 
				//	ErrorCode ); 
			...
			//DbgPrintEx( "%u.%u.%u.%u/n", 
			//	HIWORD( _FixedFileInfo->dwFileVersionMS ),
			//	LOWORD( _FixedFileInfo->dwFileVersionMS ),
			//	HIWORD( _FixedFileInfo->dwFileVersionLS ),
			//	LOWORD( _FixedFileInfo->dwFileVersionLS ) ); 
			...
			if( FixedFileInfoSize < sizeof( *FixedFileInfo ) )
			{
				...
				{
					//DbgPrintEx( "get the size %u of the file %ws version information too small\n", 
					//	FileName, 
					//	FixedFileInfoSize ); 
...
LRESULT WINAPI hex2text(IN PVOID data,IN DWORD data_len,IN OUT LPWSTR text,IN DWORD cc_buf_len)
{
	...
	do 
	{
		...
		*text = L'\0'; 
		...
		{
			StringCbPrintfW( TempBuffer, sizeof( TempBuffer ), L"%02x",DataOut[i]);
			StringCbCatW(text,cc_buf_len,TempBuffer);
		}
	} while ( FALSE );
								  ...
								  ULONG cc_pool_len )
{
	...
	ULONG readed; 
	LPWSTR terminator;	
	ULONG cc_searched_size = 0;
	INT32 terminate_pos;
	INT32 cc_terminate_pos = -1;
	ULONG _cc_remain_len = *cc_remain_len;
	ULONG _cc_max_line_len = cc_pool_len;
	INT32 line_end_index;

	ASSERT(file != INVALID_HANDLE_VALUE && cc_remain_len != NULL);
	ASSERT(cc_pool_len != 0);

	terminator = ( LPWSTR )LINE_TERM;

	do
	{
		if( _cc_remain_len > 0 )
		{
			terminate_pos = memmem( ( CHAR* )terminator, 
				( LINE_TERM_LEN << 1 ), 
				( CHAR* )( pool_backup + cc_searched_size ), 
				( ( _cc_remain_len - cc_searched_size ) << 1 ) ); 

			ASSERT( 0 == ( terminate_pos % 2 ) ); 

			cc_terminate_pos = ( terminate_pos >> 1 ); 

			if( cc_terminate_pos >= 0 )
			{
				cc_terminate_pos += cc_searched_size;
			}

			if( pool_backup[ _cc_remain_len - 1 ] == terminator[ 0 ] )
				cc_searched_size = _cc_remain_len - 1;
			else
				cc_searched_size = _cc_remain_len;

		}

		if( _cc_remain_len <= 0 || cc_terminate_pos < 0 )
		{
			if( ( _cc_max_line_len - _cc_remain_len ) > 0 )
			{

				ret = ReadFile( file, 
					( LPVOID )( pool_backup + _cc_remain_len ), 
					( ( _cc_max_line_len - _cc_remain_len ) << 1 ), 
					&readed, 
					NULL ); 
	...
	}while( cc_terminate_pos < 0 ); 

	ASSERT( cc_terminate_pos >= 0 
		&& cc_terminate_pos <= cc_pool_len ); 

	memcpy( line, 
		pool_backup, 
		( cc_terminate_pos << 1 ) ); 

	*( line + cc_terminate_pos )= L'\0';

	line_end_index = cc_terminate_pos + LINE_TERM_LEN;

	memcpy( pool_backup, 
		pool_backup + line_end_index, 
		( ( _cc_remain_len - line_end_index ) << 1 ) ); 

	memset( pool_backup + _cc_remain_len - line_end_index, 
		0, 
		( line_end_index << 1 ) ); 

	*cc_remain_len = _cc_remain_len - line_end_index;

	return cc_terminate_pos;
}
...
INT32 WINAPI read_line_continue( HANDLE file, LPSTR line, LPSTR pool_backup, ULONG *remain_len, ULONG buf_len )
{
	...
	CHAR *terminator;	
	ULONG dwSize = 0;
	INT32 terminate_pos = -1;
	ULONG _remain_len = *remain_len;
	ULONG _max_line_len = buf_len;
	INT32 line_end_index;

	ASSERT(file != INVALID_HANDLE_VALUE && remain_len != NULL);
	ASSERT(buf_len != 0);

	terminator = ( LPSTR )LINE_TERM;

	do
	{
		if(_remain_len > 0)
		{
			terminate_pos = memmem( ( LPCSTR )terminator, LINE_TERM_LEN, ( LPCSTR )pool_backup + dwSize, _remain_len - dwSize);
			if(terminate_pos >= 0)
			{
				terminate_pos += dwSize;
			}

			if(pool_backup[_remain_len - 1] == terminator[0])
				dwSize = _remain_len - 1;
			else
				dwSize = _remain_len;

		}

		if(_remain_len <= 0 || terminate_pos < 0)
		{
			if((_max_line_len - _remain_len) > 0 )
			{

				ret = ReadFile( file, pool_backup + _remain_len, _max_line_len - _remain_len, &readed, NULL );
				if( ret == FALSE || readed == 0 )
				{
					*line = 0;
					return ERR_DISCONNECT;
	...
	}while(terminate_pos < 0);

	memcpy(line, pool_backup, terminate_pos);
	*( line + terminate_pos )= 0;

	line_end_index = terminate_pos + LINE_TERM_LEN;

	memcpy(pool_backup, pool_backup + line_end_index, _remain_len - line_end_index );
	memset( pool_backup + _remain_len - line_end_index, 0, line_end_index ); 

	*remain_len = _remain_len - line_end_index;
